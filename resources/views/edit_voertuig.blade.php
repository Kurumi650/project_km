        ...
        <main class="flex-1 p-5 bg-white flex justify-center items-center">
            <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
                <div class="bg-gray-50 p-4 mb-6 rounded-lg shadow-md">
                    <h2 class="text-xl font-semibold text-gray-800 text-center">Voertuig Bewerken</h2>
                </div>
                <form action="/voertuig_opslaan/{{ $voertuig->id }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="kenteken" class="block text-sm font-semibold text-gray-700">Kenteken</label>
                        <input type="text" id="kenteken" name="kenteken" value="{{ $voertuig->kenteken }}" class="w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    </div>
                    <div class="mb-4">
                        <label for="merk" class="block text-sm font-semibold text-gray-700">Merk</label>
                        <input type="text" id="merk" name="merk" value="{{ $voertuig->merk }}" class="w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    </div>
                    <div class="mb-4">
                        <label for="model" class="block text-sm font-semibold text-gray-700">Model</label>
                        <input type="text" id="model" name="model" value="{{ $voertuig->model }}" class="w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    </div>
                    <div class="mb-4">
                        ...
                        <select id="bouwjaar" name="bouwjaar" class="w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                            ...
                            for ($year = 2024; $year >= 2006; $year--) {
                                $selected = $voertuig->bouwjaar == $year ? 'selected' : '';
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" id="hudig_km" name="hudig_km" value="{{ $voertuig->hudig_km }}" class="w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" readonly>
                    </div>
                    
                    <div>
                        <input type="submit" value="Bijwerken" class="w-full p-2 bg-blue-600 text-white font-bold rounded-md cursor-pointer hover:bg-blue-700 transition duration-300">
                    </div>
                </form>
            </div>
        </main>
